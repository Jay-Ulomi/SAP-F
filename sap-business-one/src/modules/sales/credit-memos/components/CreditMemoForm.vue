<template>
  <form @submit.prevent="handleSubmit" class="space-y-6">
    ...
    <div class="bg-white rounded-lg shadow p-4">
      <h3 class="text-base font-medium text-gray-900 mb-4">Credit Memo</h3>
      ...
      <div class="flex items-center justify-between mb-4">
        <div class="flex items-center space-x-4">
          ...
          <div class="flex items-center space-x-2">
            <label for="creditMemoType" class="text-sm font-medium text-gray-700">
              Type:
            </label>
            <select
              id="creditMemoType"
        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr v-for="(item, index) in formData.lineItems" :key="`item-${index}`">
              ...
              <td class="min-w-[120px] px-2 py-1 hidden md:table-cell">
                <select
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (props.isEditing && props.initialData?.id) {
      result = await store.updateCreditMemo(props.initialData.id, formData.value)
    } else {
      result = await store.createCreditMemo(formData.value)
    }
