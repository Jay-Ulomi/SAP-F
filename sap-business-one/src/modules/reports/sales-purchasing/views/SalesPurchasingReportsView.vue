  ...
  <div class="space-y-6">
    ...
    <div class="bg-white border border-gray-200 rounded-lg p-6">
      <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h1 class="text-2xl font-bold text-gray-900">Sales & Purchasing Reports</h1>
          <p class="text-gray-600">
            Generate comprehensive sales and purchasing analytics and reports
          </p>
        </div>
      ...
      <div class="bg-white border border-gray-200 rounded-lg p-6">
        <div class="flex items-center">
          ...
          <div class="ml-4">
            ...
            <p class="text-2xl font-semibold text-gray-900">{{ formatCurrency(stats?.totalSalesValue || 0) }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white border border-gray-200 rounded-lg p-6">
        <div class="flex items-center">
          ...
          <div class="ml-4">
            ...
            <p class="text-2xl font-semibold text-gray-900">{{ formatCurrency(stats?.totalPurchaseValue || 0) }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white border border-gray-200 rounded-lg p-6">
        <div class="flex items-center">
          ...
          <div class="ml-4">
            ...
            <p class="text-2xl font-semibold text-gray-900">{{ stats?.activeReports || 0 }}</p>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div>
          ...
          >
            ...
            <option value="PURCHASE_ANALYSIS">Purchase Analysis</option>
    ...
    <div class="bg-white border border-gray-200 rounded-lg shadow">
      <div class="px-6 py-4 border-b border-gray-200">
        <h2 class="text-lg font-medium text-gray-900">Sales & Purchasing Reports</h2>
      </div>
      ...
      <div v-else-if="filteredReports.length === 0" class="p-6 text-center">
        ...
        <p class="text-sm text-gray-600 mb-4">Generate your first sales & purchasing report to get started.</p>
            ...
            <tr v-for="report in filteredReports" :key="report.id" class="hover:bg-gray-50">
                ...
                <div class="flex items-center">
                  <div class="flex-shrink-0 h-10 w-10">
                    <div 
                      ...
                      :class="{
                        'bg-blue-100': ['SALES_ANALYSIS', 'CUSTOMER_REPORT'].includes(report.type),
                        'bg-purple-100': ['PURCHASE_ANALYSIS', 'VENDOR_REPORT'].includes(report.type),
                        'bg-green-100': ['PROFITABILITY', 'PERFORMANCE'].includes(report.type),
                        'bg-yellow-100': ['FORECAST', 'COMPARISON'].includes(report.type)
                      }"
                    >
                      <svg 
                        class="w-5 h-5"
                        :class="{
                          'text-blue-600': ['SALES_ANALYSIS', 'CUSTOMER_REPORT'].includes(report.type),
                          'text-purple-600': ['PURCHASE_ANALYSIS', 'VENDOR_REPORT'].includes(report.type),
                          'text-green-600': ['PROFITABILITY', 'PERFORMANCE'].includes(report.type),
                          'text-yellow-600': ['FORECAST', 'COMPARISON'].includes(report.type)
                        }"
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <span 
                  ...
                  :class="{
                    'bg-blue-100 text-blue-800': ['SALES_ANALYSIS', 'CUSTOMER_REPORT'].includes(report.type),
                    'bg-purple-100 text-purple-800': ['PURCHASE_ANALYSIS', 'VENDOR_REPORT'].includes(report.type),
                    'bg-green-100 text-green-800': ['PROFITABILITY', 'PERFORMANCE'].includes(report.type),
                    'bg-yellow-100 text-yellow-800': ['FORECAST', 'COMPARISON'].includes(report.type)
                  }"
                >
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                <div class="font-medium">{{ formatCurrency(report.totalValue) }}</div>
                <div class="text-xs text-gray-500">{{ report.recordCount || 0 }} records</div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
          ...
          <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            <div class="flex items-center justify-between mb-4">
              <h3 class="text-lg leading-6 font-medium text-gray-900">
                {{ reportToEdit ? 'Edit Report' : 'Generate New Sales & Purchasing Report' }}
              </h3>
            ...
            <div class="text-center py-8 text-gray-500">Report Form Component Will Be Here</div>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-7xl sm:w-full">
          <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            ...
            <div v-if="selectedReport" class="text-center py-8 text-gray-500">Report Details Component Will Be Here</div>
          </div>
        </div>
      </div>
    </div>
...
// Interfaces
interface SalesPurchasingReport {
  id: string
  name: string
  description?: string
  type: string
...
interface SalesPurchasingStats {
  totalReports: number
...
// Mock data
const mockReports: SalesPurchasingReport[] = [
  {
    id: '1',
    name: 'Monthly Sales Analysis',
    description: 'Comprehensive sales performance analysis for current month',
    type: 'SALES_ANALYSIS',
    status: 'COMPLETED',
    period: 'THIS_MONTH',
    ...
    recordCount: 387,
    parameters: { includeReturns: true, groupBy: 'SALESPERSON' }
  },
  {
    id: '2',
    name: 'Quarterly Purchase Report',
    description: 'Purchase analysis and vendor performance report',
    type: 'PURCHASE_ANALYSIS',
    status: 'COMPLETED',
    period: 'THIS_QUARTER',
    generatedDate: '2024-01-14T09:15:00Z',
    totalValue: 897000.00,
    recordCount: 245,
    parameters: { includeDiscounts: true, groupBy: 'VENDOR' }
  },
  {
    id: '3',
    name: 'Customer Profitability Report',
    description: 'Analysis of customer profitability and trends',
    type: 'PROFITABILITY',
    status: 'RUNNING',
    period: 'THIS_YEAR',
    generatedDate: '2024-01-13T08:45:00Z',
    totalValue: 2456000.00,
    recordCount: 1234,
    parameters: { includeMargins: true, minProfitMargin: 10 }
  },
  {
    id: '4',
    name: 'Sales Forecast Report',
    description: 'Sales forecasting based on historical data',
    type: 'FORECAST',
    status: 'SCHEDULED',
    period: 'NEXT_QUARTER',
    generatedDate: '2024-01-12T14:20:00Z',
    totalValue: 1567000.00,
    recordCount: 0,
    parameters: { forecastPeriod: 'QUARTERLY', method: 'LINEAR_REGRESSION' }
  },
  {
    id: '5',
    name: 'Vendor Performance Analysis',
    description: 'Detailed analysis of vendor delivery and quality metrics',
    type: 'VENDOR_REPORT',
    status: 'COMPLETED',
    period: 'LAST_QUARTER',
    generatedDate: '2024-01-11T16:45:00Z',
    totalValue: 756000.00,
    recordCount: 89,
    parameters: { includeQualityMetrics: true, minimumOrderValue: 1000 }
  }
]

const mockStats: SalesPurchasingStats = {
  totalReports: 28,
  totalSalesValue: 4567890.00,
  totalPurchaseValue: 2345678.00,
  activeReports: 15
}

// Local state
const reports = ref<SalesPurchasingReport[]>([...mockReports])
const loading = ref(false)
const error = ref<string | null>(null)
const stats = ref<SalesPurchasingStats>({ ...mockStats })
const formLoading = ref(false)

const showCreateModal = ref(false)
const showViewModal = ref(false)
const reportToEdit = ref<SalesPurchasingReport | null>(null)
const selectedReport = ref<SalesPurchasingReport | null>(null)
...
const viewReport = (report: SalesPurchasingReport) => {
...
const editReport = (report: SalesPurchasingReport) => {
  reportToEdit.value = report
  showCreateModal.value = true
}

const downloadReport = (report: SalesPurchasingReport) => {
  // Simulate download
...
const deleteReport = async (id: string) => {
  if (confirm('Are you sure you want to delete this report?')) {
    try {
      ...
      if (reports.value.find(r => r.id === id && r.status !== 'COMPLETED')) {
        stats.value.activeReports--
      }
    } catch (err) {
...
const handleSubmitReport = async (reportData: any) => {
  try {
    ...
    if (reportToEdit.value) {
      ...
      // Create new report
      const newReport: SalesPurchasingReport = {
