<template>
  <div class="p-6 bg-gray-50 min-h-screen">
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-3xl font-bold text-gray-900">Incoming Payments</h1>
      <button
      ...
      <div class="bg-white p-4 rounded-lg shadow border border-gray-200">
        <div class="flex items-center">
          ...
          <div class="ml-4">
            ...
            <p class="text-xl font-bold text-gray-900">{{ stats.clearedPayments || 0 }}</p>
          </div>
        </div>
      </div>
    </div>
    ...
    >
      ...
      >
        ...
        <PaymentView :payment="viewingPayment" />
      </div>
    </div>
  </div>
</template>
...
import { useIncomingPaymentsStore } from '../stores'
const PaymentForm = defineAsyncComponent(() => import('../components/PaymentForm.vue'))
const PaymentView = defineAsyncComponent(() => import('../components/PaymentView.vue'))
import type { IncomingPayment, IncomingPaymentFormData } from '../types'

const paymentsStore = useIncomingPaymentsStore()
...
const stats = computed(() => ({
  ...
  totalAmount: payments.value.reduce((sum: number, p: IncomingPayment) => sum + p.amount, 0),
  pendingPayments: payments.value.filter((p: IncomingPayment) => p.status === 'PENDING').length,
  clearedPayments: payments.value.filter((p: IncomingPayment) => p.status === 'CLEARED').length,
}))
...
const handleSubmitPayment = async (data: IncomingPaymentFormData) => {
  try {
