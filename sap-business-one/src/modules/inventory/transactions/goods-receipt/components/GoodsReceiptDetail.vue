  ...
  <div class="bg-white rounded-lg shadow-sm border border-gray-200">
    ...
    <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
      <div class="flex items-center justify-between">
        <div>
          <h2 class="text-xl font-semibold text-gray-900">{{ receipt.documentNo }}</h2>
          <p class="text-sm text-gray-600 mt-1">{{ receipt.supplier }}</p>
          <p class="text-sm text-gray-500 mt-1">Posting Date: {{ formatDate(receipt.postingDate) }}</p>
        </div>
        <div class="flex items-center space-x-3">
          <span
            class="inline-flex px-3 py-1 text-sm font-medium rounded-full"
            :class="{
              'bg-green-100 text-green-800': receipt.status === 'Received',
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <div class="bg-gray-50 rounded-lg p-4">
            <h3 class="text-sm font-medium text-gray-900 uppercase tracking-wide mb-3">Receipt Information</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <span class="text-xs text-gray-500">Document No:</span>
                <span class="ml-2 text-sm font-medium text-gray-900">{{ receipt.documentNo }}</span>
              </div>
              <div>
                <span class="text-xs text-gray-500">Series:</span>
                <span class="ml-2 text-sm font-medium text-gray-900">{{ receipt.series }}</span>
              </div>
              <div>
                <span class="text-xs text-gray-500">Posting Date:</span>
                <span class="ml-2 text-sm font-medium text-gray-900">{{ formatDate(receipt.postingDate) }}</span>
              </div>
              <div>
                <span class="text-xs text-gray-500">Document Date:</span>
                <span class="ml-2 text-sm font-medium text-gray-900">{{ formatDate(receipt.documentDate) }}</span>
              </div>
              <div>
                <span class="text-xs text-gray-500">Business Partner Code:</span>
                <span class="ml-2 text-sm font-medium text-gray-900">{{ receipt.businessPartnerCode }}</span>
              </div>
              <div>
                <span class="text-xs text-gray-500">Business Partner Name:</span>
                <span class="ml-2 text-sm font-medium text-gray-900">{{ receipt.businessPartnerName }}</span>
              </div>
              <div>
                <span class="text-xs text-gray-500">Supplier:</span>
                <span class="ml-2 text-sm font-medium text-gray-900">{{ receipt.supplier }}</span>
              </div>
              <div>
                <span class="text-xs text-gray-500">Warehouse:</span>
                <span class="ml-2 text-sm font-medium text-gray-900">{{ receipt.warehouse }}</span>
              </div>
            </div>
          </div>
          ...
          <!-- Line Items -->
          <div class="bg-gray-50 rounded-lg p-4">
            <h3 class="text-sm font-medium text-gray-900 uppercase tracking-wide mb-3">Receipt Items</h3>
            <div class="overflow-x-auto">
              <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  <tr v-for="item in receipt.lineItems" :key="item.itemCode">
          ...
          <div class="bg-gray-50 rounded-lg p-4" v-if="hasAccountingDimensions">
            ...
            <div class="overflow-x-auto">
              <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  <tr v-for="item in receipt.lineItems" :key="`dim-${item.itemCode}`">
                    <td class="px-3 py-2 whitespace-nowrap text-sm font-medium text-gray-900">
                      {{ item.itemCode }}
                    </td>
                    <td class="px-3 py-2 whitespace-nowrap text-sm text-gray-500">
        ...
        <div class="space-y-6">
          ...
          <div class="bg-gray-50 rounded-lg p-4">
            ...
            <div class="space-y-2">
              <div class="flex justify-between">
                ...
                <span class="text-sm font-medium text-gray-900">{{ receipt.itemsCount }}</span>
              </div>
              <div class="flex justify-between">
              ...
              <div class="flex justify-between border-t border-gray-200 pt-2">
                ...
                <span class="text-base font-semibold text-gray-900">{{ formatCurrency(receipt.totalAmount) }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="bg-gray-50 rounded-lg p-4">
            ...
            <div class="space-y-2">
              ...
              <div class="flex justify-between" v-if="receipt.createdAt">
                <span class="text-sm text-gray-600">Created:</span>
                <span class="text-sm font-medium text-gray-900">{{ formatDateTime(receipt.createdAt) }}</span>
              </div>
              <div class="flex justify-between" v-if="receipt.updatedAt">
                <span class="text-sm text-gray-600">Last Updated:</span>
                <span class="text-sm font-medium text-gray-900">{{ formatDateTime(receipt.updatedAt) }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="bg-gray-50 rounded-lg p-4">
            ...
            <div class="space-y-2">
              ...
              <button
                v-if="receipt.status === 'Pending' || receipt.status === 'Draft'"
              ...
              <button
                v-if="receipt.status !== 'Received' && receipt.status !== 'Cancelled'"
...
const emit = defineEmits<{
  edit: [receipt: GoodsReceipt]
  complete: [receipt: GoodsReceipt]
  cancel: [receipt: GoodsReceipt]
  print: [receipt: GoodsReceipt]
}>()

// Computed
const subtotal = computed(() => {
  return props.receipt.lineItems.reduce((sum, item) => sum + item.lineTotal, 0)
})

const hasAccountingDimensions = computed(() => {
  return props.receipt.lineItems.some(item =>
