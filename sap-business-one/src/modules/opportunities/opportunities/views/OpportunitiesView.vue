  ...
  <div class="p-6 bg-gray-50 min-h-screen">
            ...
            <tr v-for="opportunity in opportunities" :key="opportunity.id">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <span
                  ...
                  :class="{
                    ...
                    'bg-slate-100 text-slate-800': opportunity.status === 'ON_HOLD',
                  }"
                >
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <div class="flex items-center space-x-2">
                  ...
                  <button
                    @click="duplicateOpportunity(opportunity)"
    ...
    >
      ...
      >
        ...
        <OpportunityView v-if="selectedOpportunity" :opportunity="selectedOpportunity" />
      </div>
    </div>
      ...
      >
        ...
        <form @submit.prevent="handleDuplicateOpportunity" class="space-y-4">
          ...
          <div class="flex justify-end space-x-3 pt-4">
            ...
            <button
              type="submit"
              class="px-4 py-2 text-sm font-medium text-white bg-sap-blue border border-transparent rounded-md shadow-sm hover:bg-sap-dark-blue focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sap-blue"
            >
              Duplicate
            </button>
          </div>
...
const duplicateOpportunity = (opportunity: Opportunity) => {
  editingOpportunity.value = opportunity
  duplicateForm.value.opportunityCode = `${opportunity.opportunityCode}-COPY`
  duplicateForm.value.title = `${opportunity.title} (Copy)`
  duplicateForm.value.customerName = opportunity.customerName
...
const handleDuplicateOpportunity = async () => {
  ...
  try {
    await opportunityStore.duplicateOpportunity(editingOpportunity.value.id)
    closeDuplicateModal()
    await loadData()
  } catch (err) {
...
const applyFilters = async () => {
  ...
  if (filters.value.status) {
    newFilters.status = filters.value.status as OpportunityStatus
  }

  if (filters.value.stage) {
    newFilters.stage = filters.value.stage as OpportunityStage
  }

  if (filters.value.type) {
    newFilters.type = filters.value.type as OpportunityType
  }

  if (filters.value.priority) {
    newFilters.priority = filters.value.priority as OpportunityPriority
  }

  opportunityStore.setFilters(newFilters)
  await opportunityStore.loadOpportunities(1, 10, newFilters)
}
...
const nextPage = async () => {
  if (pagination.value.hasNext) {
    await opportunityStore.loadOpportunities(pagination.value.page + 1)
  }
}
...
const goToPage = async (page: number) => {
  await opportunityStore.loadOpportunities(page)
}
...
const loadData = async () => {
  await opportunityStore.loadOpportunities()
  await opportunityStore.loadStats()
}
